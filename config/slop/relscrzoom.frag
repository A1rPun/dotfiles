...
void main()
{
    ...
    vec2 boxOffset = vec2(-0.25,-0.25) * screenSize;
    vec2 boxSize = vec2(0.5,0.5) * screenSize;
    float magstrength = 4;
    vec2 borderSize = vec2(2,2);
    vec4 borderColor = vec4(0.22,0.57,0.96,1);
    bool crosshair = true;
    //------------------------------
    ...
    if ( uvCoord.x < mpos.x+boxOffset.x+boxSize.x &&
      ...
      if ( (distance(uvCoord.x, mpos.x+boxOffset.x+boxSize.x/2) <= borderSize.x/2 ||
            distance(uvCoord.y, mpos.y+boxOffset.y+boxSize.y/2) <= borderSize.y/2) && crosshair ) {
        color = borderColor;
      } else {
